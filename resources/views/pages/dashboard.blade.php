...
@push('js')
    ...
    <script>
            ...
            function fetchDataAndDrawChart(chart) { // Pass chart as an argument
                const urls = [
                    '{{ route("totalPlayersInIniciacao") }}',
                    '{{ route("totalPlayersInPetizes") }}',
                    '{{ route("totalPlayersInTraquinas") }}', // Corrected typo
                    '{{ route("totalPlayersInBenjamins") }}',
                    '{{ route("totalPlayersInInfantis") }}',
                    '{{ route("totalPlayersInIniciados") }}',
                    '{{ route("totalPlayersInJuvenis") }}',
                    '{{ route("totalPlayersInJuniores") }}',
                    '{{ route("totalPlayersInSeniores") }}'
                ];
                ...
                Promise.all(promises)
                    .then(data => {
                        data.forEach((response, index) => {
                            if (response.hasOwnProperty('totalPlayers')) {
                                chart.data.datasets[0].data[index] = response.totalPlayers;
                            } else {
                                console.error("Missing 'totalPlayers' property in response:", response);
                            }
                        });
                        chart.update();
                    })
